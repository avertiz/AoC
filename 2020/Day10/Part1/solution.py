...
def get_adapter_joltage_rate(adapters):
    adapter_list = adapters['adapter'].tolist()
    max_joltage = max(adapter_list)
    adapter_joltage_rate = max_joltage + 3
    return(adapter_joltage_rate)

def accept_joltage(adapter, current_joltage_rating):
    if adapter - current_joltage_rating <= 3 and adapter - current_joltage_rating >= 0:
        return(True)
    else:
        return(False)

def get_jolage_difference(previous_adapter, current_adapter):
    diff = current_adapter - previous_adapter
    return(diff)

def main():
    input = pd.read_csv('input.csv', skip_blank_lines=False)
    input.sort_values(by='adapter', inplace = True)
    adapter_joltage_rate = get_adapter_joltage_rate(adapters = input)
    difference_list = []
    current_joltage_rating = 0
    previous_adapter = 0
    for key, row in input.iterrows():
        if accept_joltage(adapter = row['adapter'], current_joltage_rating = current_joltage_rating):
            current_adapter = row['adapter']
            joltage_difference = get_jolage_difference(previous_adapter = previous_adapter, current_adapter = current_adapter)
            difference_list.append(joltage_difference)
            current_joltage_rating = current_adapter
            previous_adapter = copy.copy(current_adapter)
    difference_list.append(3) # for my adapter
    difference_list = [str(int) for int in difference_list]
    one_jolt_differences = difference_list.count('1')
    three_jolt_differences = difference_list.count('3')
    answer = one_jolt_differences * three_jolt_differences
    print(answer)
