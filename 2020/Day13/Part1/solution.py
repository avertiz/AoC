...
# When you call to book another ship, you discover that no ships embark from that port to your vacation island. You'll need to get from the port to the nearest airport.
...
def get_earliest_timestamp(df):
    earliest_timestamp = int(df['input'][0])
...
def get_pickup_time(earliest_timestamp, id):
    if earliest_timestamp % id == 0:
        pickup_time = earliest_timestamp
    else:
        pickup_time = earliest_timestamp - (earliest_timestamp % id) + id
    return(pickup_time)

def main():
    input = pd.read_csv('input.csv', skip_blank_lines=False, header=None, names = ['input'])
    earliest_timestamp = get_earliest_timestamp(df = input)
    ...
    for id in ids:
        if id == 'x':
            ...
            id = int(id)
            pickup_time_dict[id] = get_pickup_time(earliest_timestamp = earliest_timestamp, id = id)
    for key in pickup_time_dict.keys():
        if pickup_time_dict[key] == min(pickup_time_dict.values()):
            bus_id = key
    answer = (pickup_time_dict[bus_id] - earliest_timestamp) * bus_id
    print(answer)
