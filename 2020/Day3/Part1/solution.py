...
def traverse(df, row_num, position, trees):
    ...
    if row_num == df.shape[0]-1:
        ...
        if row[position] == '#':
            trees += 1
        return traverse(df = df, row_num = row_num + 1, position = position + 3, trees = trees)

def main():
    input = pd.read_csv('input.csv')
    trees = traverse(df = input, row_num = 0, position = 0, trees = 0)
    print(trees)
