...
class Solution:
    ...
    def flash(self, row, col):
        ...
        # we only do so if they have not already flashed
        try:
            # right
            if self.octopuses[row][col + 1] != 0:
                self.octopuses[row][col + 1] += 1
        except IndexError:
            pass
        try:
            # bottom right
            if self.octopuses[row + 1][col + 1] != 0:
                self.octopuses[row + 1][col + 1] += 1
        except IndexError:
            pass
        try:
            # bottom
            if self.octopuses[row + 1][col] != 0:
                self.octopuses[row + 1][col] += 1
        except IndexError:
            pass
        try:
            # bottom left
            if col > 0 and self.octopuses[row + 1][col - 1] != 0:
                self.octopuses[row + 1][col - 1] += 1
        except IndexError:
            pass
        try:
            # left
            if col > 0 and self.octopuses[row][col - 1] != 0:
                self.octopuses[row][col - 1] += 1
        except IndexError:
            pass
        try:
            # top left
            if col > 0 and row > 0 and self.octopuses[row - 1][col - 1] != 0:
                self.octopuses[row - 1][col - 1] += 1
        except IndexError:
            pass
        try:
            # top
            if row > 0 and self.octopuses[row - 1][col] != 0:
                self.octopuses[row - 1][col] += 1
        except IndexError:
            pass        
...
def main():
    ...
    for i in range(100):
        ...
        flashers_exist = data.flasher_check()
        while flashers_exist:
            data.get_flashers()
